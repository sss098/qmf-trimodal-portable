...
class Config:
    ...
    def validate(self) -> None:
        active = self.modalities.split(",")
        canonical = [m for m in ("us", "emg", "table") if m in active]
        if active != canonical or not active:
            raise ValueError("modalities must be a nonempty ordered subset of us,emg,table")
