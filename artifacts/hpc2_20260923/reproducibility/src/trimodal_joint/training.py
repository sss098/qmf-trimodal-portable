...
def fit(
    ...
    model = TrimodalModel(data.table.shape[1], config.imagenet_weights, config.modalities).to(
        device
    )
    groups = []
    if "us" in model.modalities:
        groups.append({"params": model.ultrasound.parameters(), "lr": config.encoder_lr})
    other = [p for name, p in model.named_parameters() if not name.startswith("ultrasound.")]
    groups.append({"params": other, "lr": config.lr})
    optimizer = torch.optim.AdamW(
        groups, weight_decay=config.weight_decay, betas=(0.9, 0.999), eps=1e-8
    )
    ...
    quality = QualityHistory(len(train), len(model.modalities)) if method == "qmf" else None
    last = folder / "last.pt"
    best = float("inf")
    selected = 0
    stale = 0
...
def train_runs(
    ...
    if len(config.modalities.split(",")) == 1 and methods != ["equal_late"]:
        raise ValueError("Single-modality runs require --methods equal_late")
