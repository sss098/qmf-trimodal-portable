...
class QualityHistory:
    ...
    def __init__(self, patients: int):
        self.sums = torch.zeros(patients, 3, dtype=torch.float64)
...
def total_loss(
    ...
    if method == "tmc":
        return evidential_loss(out["alpha"], y, epoch, weights) + sum(
            evidential_loss(out["branch_alpha"][:, m], y, epoch, weights) for m in range(3)
        )
    branch_ce = torch.stack(
        [F.cross_entropy(branches[:, m], y, reduction="none") for m in range(3)], 1
    )
    loss = F.cross_entropy(out["scores"], y, weight=weights) + sum(
        weighted_mean(branch_ce[:, m], y, weights) for m in range(3)
    )
